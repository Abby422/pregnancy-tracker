  ...
  const babyData = () => {
   ...
   useEffect(() => {
     ...
     const setUserDisplayName = async () => {
       if (userId) {
         const user = await fetchUserData(userId); // You may need to implement a function to get user data

         console.log('User: ' + user);
         setUserName(user?.email || ""); // Adjust the field name based on your actual data structure
       }
     };
